...
export async function createQVICredential(
  ...
  // Try to delete notification, but don't fail if it doesn't exist
  try {
    await client.notifications().delete(grantNotification.i);
  } catch (e: any) {
    console.log(
      "Could not delete QVI notification (may not exist):",
      e.message
    );
  }
...
/**
 * Check if KERI client is properly authenticated
 * @param client SignifyClient instance
 * @returns Promise<boolean> - true if authenticated, false otherwise
 */
export async function isKERIAuthenticated(
  client: SignifyClient
): Promise<boolean> {
  try {
    await client.state();
    ...
    console.error("❌ KERI authentication check failed:", error.message);
    return false;
  }
}

/**
 * Re-establish KERI authentication by reconnecting
 * @param client SignifyClient instance
 * @param bran Authentication bran
 * @returns Promise<SignifyClient> - Re-authenticated client
 */
export async function reauthenticateKERI(
  client: SignifyClient,
  bran: string
): Promise<SignifyClient> {
  try {
    console.log("🔄 Re-authenticating KERI client...");
    ...
    console.log(`✅ KERI re-authentication successful. AID: ${aid}`);

    return client;
  } catch (error: any) {
    console.error("❌ KERI re-authentication failed:", error.message);
    throw new Error(`Failed to re-authenticate KERI: ${error.message}`);
  }
}
